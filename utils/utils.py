...
def compute_return(speed):
	global cumsumgain
	if cumsumgain is None:
		cumsumgain = torch.arange(speed.shape[1],0,-1).unsqueeze(0).unsqueeze(-1).unsqueeze(-1).cuda()
...
def compute_uncertainty(data,mean):
	return torch.max(torch.abs(data-mean),dim=0)[0].unsqueeze(0)
